...
print('Скрипт запущен.\n'
      'Для прекращения сбора статистики закройте окно или приложение.\n'
      '\n'
...
with open(f'statistic files/{p.name()} {now.strftime("%d-%m-%Y %H-%M")}.csv', 'a', newline='') as csv_file:
    ...
    csv_writer.writerow(['Загрузка CPU', 'Потребление памяти: Private Bytes', 'Потребление памяти: Working Set',
                         'Количество открытых хендлов'])

while True:
    with p.oneshot():
        with open(f'statistic files/{p.name()} {now.strftime("%d-%m-%Y %H-%M")}.csv', 'a', encoding='UTF-8',
                  newline='') as csv_file:
            csv_writer = csv.writer(csv_file, delimiter=';')
            csv_writer.writerow([p.cpu_percent(interval=n), p.memory_info().private / 1024,
                                 p.memory_info().wset / 1024, p.num_handles()])
